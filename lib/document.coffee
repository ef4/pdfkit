...
class PDFDocument extends stream.Readable
  ...
  addPage: (options = @options) ->
    ...
    @page?.end() unless @_pageBuffer

    # create a page object
    @page = new PDFPage(this, options)

    # optional page buffering
    @_pageBuffer.push(@page) if @_pageBuffer
    console.log('buffered new page')
  ...
  _assertBufferedMode: ->
    unless @_pageBuffer
      throw new Error("flushPages is only supported when using bufferPages mode")
  ...
  switchToPage: (n) ->
    @_assertBufferedMode()
    unless page = @_pageBuffer[n]
      throw new Error("switchToPage(#{n}) out of bounds")
    @page = page
  ...
  ref: (data) ->
    ...
    @_waiting++
    console.log('incremented waiting to ', @_waiting)
    return ref
  ...
  _refEnd: (ref) ->
    ...
    console.log('decrementing waiting from ', @_waiting)
