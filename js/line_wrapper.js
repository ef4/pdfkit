...
(function() {
  ...
  LineWrapper = (function(_super) {
    ...
    LineWrapper.prototype.trailingSpaceWidth = function(word) {
      var match;
      match = /(\s+)$/.exec(word);
      if (match) {
        return this.wordWidth(match[1]);
      } else {
        return 0;
      }
    };
    ...
    LineWrapper.prototype.eachWord = function(text, fn) {
      var bk, breaker, fbk, l, last, lbk, shouldContinue, trailing, w, word, wordWidths;
      ...
      while (bk = breaker.nextBreak()) {
        ...
        trailing = this.trailingSpaceWidth(word);
        if (w - trailing > this.lineWidth + this.continuedX) {
          lbk = last;
          fbk = {};
          while (word.length) {
            l = word.length;
    ...
    LineWrapper.prototype.wrap = function(text, options) {
      ...
      this.eachWord(text, (function(_this) {
        return function(word, w, bk, last, trailing) {
          var lh, shouldContinue;
          if ((last == null) || last.required) {
            _this.emit('firstLine', options, _this);
            _this.spaceLeft = _this.lineWidth;
          }
          if (w - trailing <= _this.spaceLeft) {
            buffer += word;
            textWidth += w;
            wc++;
          }
          if (bk.required || w - trailing > _this.spaceLeft) {
            if (bk.required) {
